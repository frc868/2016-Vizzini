 ...
 */
public class SynchronousPIDWPI {
	...
	public SynchronousPIDWPI(double Kp, double Ki, double Kd, double Kf, PIDSourceType pidType) {
		m_P = Kp;
		m_I = Ki;
		m_D = Kd;
		m_F = Kf;
	...
	public SynchronousPIDWPI(double Kp, double Ki, double Kd, PIDSourceType pidType) {
		this(Kp, Ki, Kd, 0.0, pidType);
	}
	...
	public double calculate(double currentInput) {
		...
		synchronized (this) {
			...
			if (m_pidType.equals(PIDSourceType.kRate)) {
				if (m_P != 0) {
